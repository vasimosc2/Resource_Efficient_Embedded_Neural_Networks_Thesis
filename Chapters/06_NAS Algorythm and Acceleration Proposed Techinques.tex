...
\TODO{CHAT}
We emphasize that our \emph{fitness function} is the final test-set accuracy of each architecture. The above rules only use validation accuracy to guide early stopping, so the overall selection based on test accuracy is unaffected. Prior work shows that such stopping can greatly reduce compute without hurting accuracy. For example, Aach *et al.* (2023) report that using performance-based early stopping in parallel NAS yields roughly $2$–$5\times$ speedups \emph{while preserving test-set accuracy}.

In their experiments, the final validation accuracies of models found with early stopping differed by less than $1\%$ from those without stopping. Similarly, Zheng *et al.* (2020) achieve on the order of $10^3\times$ faster search via aggressive budgeted stopping with only $\approx0.3\%$ accuracy loss.

These results justify our strategy: we incur at most a negligible change in final accuracy in exchange for large reductions in training time and resources.
...
\TODO{This diagram has to be changed}
